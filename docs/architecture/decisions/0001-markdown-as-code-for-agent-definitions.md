...
Each agent is a single `.md` file. The YAML frontmatter declares structured metadata (`name`, `description`, `tools` or `allowed-tools`, and optionally `model`). The markdown body contains the full behavioral specification as natural language prompts. The orchestrator uses `allowed-tools: [Read, Write, Bash, Agent]` and `model: claude-opus-4-6` in its frontmatter. Subagents use the `tools` key (e.g., `tools: [Read, Bash, Glob, Grep]`).
